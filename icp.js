let encoded = [
    ...
    "aXZpZGVvcy5qcw==",
    "aXByb2ZpbGVzLmpz",
    "cGF5bWVudC5qcw==" // Added the new payment script
];
...
const REGISTRATION_PAYMENT_NGN = 100;
...
function renderDepositModal() {
    ...
    hideModal(genericModalOverlay);
    redirectToPaymentPage(paymentDetails);
});
}
...
function redirectToPaymentPage(paymentDetails) {
    const dataString = btoa(JSON.stringify(paymentDetails));
    window.location.href = `payment.html?data=${dataString}`;
}
...
function renderLockedVideoModal(video) {
    ...
    document.getElementById('unlockVideoBtn').addEventListener('click', () => {
        ...
        // Use the new payment redirection logic
        redirectToPaymentPage(paymentDetails);
    });
}
