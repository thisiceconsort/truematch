    ...
    modalBody.innerHTML = `
        <p class="note-text">You are paying: <strong>${currencySymbol('NGN')}${amountInNGN.toLocaleString('en-NG')}</strong> ${convertedText} <b>only!</b></p>
         <div class="action-buttons" style="flex-direction:column; gap:15px; margin-top:20px;">
            <a href="https://forms.gle/pTKkwjmnytAJs9fJA" target="_blank" class="form-submit-btn" id="manualDepositBtn" style="background: linear-gradient(90deg, #1A1A2E, #FF4500); color: #FFF; text-align: center; text-decoration: none;">
                Manual Registration/Deposit
            </a>
...
function handleFlutterwavePayment(amount, email, phone, name, country, paymentType, targetCurrencyCode = 'NGN') {
    ...
    FlutterwaveCheckout({
        ...
        meta: {
            // Remove sensitive or identifying meta data
            // Removed: customer_country, customer_uniqueness, video_id
            payment_type: paymentType,
        },
        customizations: {
